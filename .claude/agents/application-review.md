...
**Check for an invented relationship, not just an invented fact.** A bullet can be built from two individually true facts and still be a fabrication, if it draws a causal or purposive link between them the source never makes. Read the exact lines in `experience.md` behind any bullet that combines more than one fact. Three shapes to watch for: two facts the source lists as separate bullets folded into one flow, a qualifier that describes one piece of work extended onto a second nearby one the source states independently, and a mechanism named as performing an outcome the source lists as a separate step. If the connective word, "to," "for," "so that," "which let," "as part of," is not itself something the source states, it is invented even though every noun in the sentence is real. This has shipped three times in four recent CVs and no script catches it.

**When Farrukh supplied or wrote the content himself**, absence from `skills.md`, `experience.md`, `education.md` or `projects.md` is not itself grounds to flag a claim as unsupported. He is the authority on his own experience, and a fact he states directly is sourced, not invented. Ask him to confirm it, then record it in the right pool file (`skills.md` at `listed` is usually right) so it is available next time, rather than treating the pipeline's silence as evidence against him. VIPER, TDD, MERN, React and Node were each real and each briefly flagged as unsupported for exactly this reason before being added to `skills.md`.
